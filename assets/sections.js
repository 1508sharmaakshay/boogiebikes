...
selectCallback = function(variant, selector) {
  ...
  if (variant && variant.available == true) {
    ...
    $('.regular_btn', $product).removeClass('hidden');
    $('.custom_button', $product).addClass('hidden');
    $notify_form.hide();
    ...
    $('.purchase-details__buttons.custom', $product).removeClass('product-is-unavailable');
    $('.regular_btn', $product).addClass('hidden');
    $('.custom_button', $product).removeClass('hidden');
...
/*============================================================================
    CTA Footer, header on a product page
==============================================================================*/

  document.addEventListener("scroll", function(){stickyFun()});

	
	
    var navbar = $('.nav-footer');
	var sticky = navbar.offsetTop;
    var mobileNavbar = $('.mobile-nav-footer');
...
document.addEventListener("scroll", function(){stickyHeader()});
	...
	var navmenu = $('.nav-header');
		
    
    function stickyHeader() {
      if ($(window).scrollTop() > $('#heightoffset').height()) {
...
/*============================================================================
    Pre-order page Pay partially scroll, payment sellect
==============================================================================*/

 	var selectNew = $('.one_pay');
    var faqLast = $('.faqAccordion > dt:last-of-type > button');
	

	$(document).ready(function(){
      selectNew.addClass("selected")
//       document.querySelector('.single-option-selector').classList.add("selected")
    });
    
    function scrollToFAQ() {
        faqLast.click()
        selectNew.removeClass("selected")
        $('.single-option-selector').removeClass("selected")
        faqLast.attr("id", "faq_payment")
        faqLast.addClass("selected")
        $("#button_scroll").addClass("selected")

        setTimeout(
            $('html,body').animate({
                scrollTop: $("#faq_payment").offset().top - 200
            },
                'slow'), 0)
    };

	function onePayOption(){
    	selectNew.addClass("selected")
        $('select.single-option-selector').addClass("selected")
        $("#button_scroll").removeClass("selected")
    }

// $('.faqAccordion dt').each(function(item){console.log(item)}) search all items
// setTimeout для определённой функции запустить скролл с задержкой

/*============================================================================
    Different devisec widget position
==============================================================================*/
...
$(document).ready(function(){
    if (isMobile.Windows() && $(window).width() < 500) {
       $('body').addClass("windows");
    }
     if (isMobile.Android() && $(window).width() < 500) {
       $('body').addClass("android");
    }
      if (isMobile.iOS() && $(window).width() < 500) {
       $('body').addClass("iOS");
    }
});
...
// $(document).ready(function(){
//   if(isMobile.Windows()) {
//   $(".iDCmXC").css("bottom","40px!important")}
// });

// window.addEventListener("message", function ({console.log(this)}));

 
 var buttonBackToSelector = $(".buy-now.scroll");
 var buttonBackToSelectorDesk = $(".buy-now.scroll-cta")

	buttonBackToSelector.click(function scrollToo(e){
       $('html, body').animate({ 
      	scrollTop: $(".payment_options").offset().top - 200
      });
      e.preventDefault();
	});

 buttonBackToSelectorDesk.click(function scrollToo(e){
       $('html, body').animate({ 
      	scrollTop: $(".payment_options").offset().top - 200
      });
      e.preventDefault();
